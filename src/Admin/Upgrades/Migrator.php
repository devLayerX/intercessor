 ...
 * @since       1.1.0
 */
namespace Intercessor\Admin\Upgrades\v3;
...
class Migrator {
		...
		if ( property_exists( $data, 'notes' ) && ! empty( $data->notes ) ) {
			...
			if ( ! empty( $notes ) ) {
				foreach ( $notes as $note ) {
					try {
						$date = isset( $note[0] )
							? Intercessor()->utils->date( $note[0], edd_get_timezone_id() )->setTimezone( 'UTC' )->toDateTimeString()
							: '';
					} catch ( \Exception $e ) {
	...
	public static function orders( $data = null ) {
		...
		// Some old Intercessor data has the user info serialized, but starting with something other than a: so it can't be unserialized
		 ...
		 * `Intercessor\Admin\Upgrades\v3\Data_Migrator::fix_possible_serialization()`
		...
		// Some old Intercessor data has the cart details serialized, but starting with something other than a: so it can't be unserialized
		...
		// Some old Intercessor data has the downloads serialized, but starting with something other than a: so it can't be unserialized
		...
		if ( ! in_array( $order_status, $non_completed_statuses ) ) {
			...
			if ( ! empty( $date_completed ) ) {  // Update the data_completed to the UTC.
				try {
					$date_completed = Intercessor()->utils->date( $date_completed, edd_get_timezone_id() )->setTimezone( 'UTC' )->toDateTimeString();
				} catch ( \Exception $e ) {
		...
		if ( ! empty( $customer ) && $customer instanceof \Intercessor_Customer ) {
		...
		if ( ! empty( $cart_details ) ) {
			foreach ( $cart_details as $key => $cart_item ) {
				...
				if ( isset( $cart_item['fees'] ) && ! empty( $cart_item['fees'] ) ) {
					foreach ( $cart_item['fees'] as $fee_id => $fee ) {
						...
						$tax   = Intercessor()->fees->get_calculated_tax( $fee, $tax_rate );
			...
			// Compatibility with older versions of Intercessor.
		...
		if ( ! empty( $discounts ) && ( 'none' !== $discounts[0] ) ) {
			if ( 1 === count( $discounts ) ) {
				...
				/** @var \Intercessor_Discount $discount_object */
				$discount_object = edd_get_discount_by( 'code', $discount_code );

				if ( $discount_object instanceof \Intercessor_Discount ) {
					$discount_args = array(
						'object_id'     => $order_id,
						'object_type'   => 'order',
					 ...
					 * @param \Intercessor_Discount $discount_object Discount object.
