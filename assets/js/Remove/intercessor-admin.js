			...
			$.ajax({
				...
				success: function( response ) {
					if ( 'done' == response.step || response.error || response.success ) {
						...
						var export_form    = $('.intercessor-export-form').find('.intercessor-progress').parent().parent();
	...
	var IPR_Import = {
		...
		before_submit : function( arr, $form, options ) {
			...
			if ( window.File && window.FileReader && window.FileList && window.Blob ) {
				...
				var import_form = $('.intercessor-import-form').find('.intercessor-progress').parent().parent();
		...
		complete: function( xhr ) {
			...
			if ( response.success ) {
				...
				var $form = $('.intercessor-import-form .notice-wrap').parent();

				$form.find('.intercessor-import-file-wrap,.notice-wrap').remove();
				$form.find('.intercessor-import-options').slideDown();

				// Show column mapping.
				var select  = $form.find('select.intercessor-import-csv-column');
...
function intercessor_attach_tooltips( selector ) {
	// Tooltips
	selector.tooltip({
		content: function() {
